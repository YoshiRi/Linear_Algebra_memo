...
W = sigma1^2;
for lpole = 1:20
    Ks = acker(sysd.A',sysd.A'*sysd.C',exp(ST*acbase*-1*lpole))';
    Cnom=Ks*W*Ks';
...
SaveFigPDF(2,strcat('statenoise_pred_acker_F',num2str(Fnum)))
...
SaveFigPDF(3,strcat('obsnoise_pred_acker_F',num2str(Fnum)))
...
SaveFigPDF(10,strcat('trace_pred_acker_F',num2str(Fnum)))
%%
final_errex_servo = zeros(20,4);
G = 10;

for lpole = 1:20
    Ks = acker(sysd.A',sysd.A'*sysd.C',exp(ST*acbase*-1*lpole))';
    Aex=[sysd.A+sysd.B*F -sysd.B*F-sysd.B*G*sysd.C;zeros(2) sysd.A-Ks*sysd.C*sysd.A];
    Kex = [-sysd.B*G;-Ks];
    Cnomex=Kex*W*Kex';
    Pexends = dlyap(Aex,Cnomex);
    final_errex_servo(lpole,:)=diag(Pexends)';
end

figure(22)
% plot(final_errex(:,1:3))
grid on
plot(lpoles,log(final_errex(:,1)),'o-',lpoles,log(final_errex(:,2)),'x-',lpoles,log(final_errex_servo(:,1)),'o-.',lpoles,log(final_errex_servo(:,2)),'x-.')
grid on
xlabel('pole frequency [rad]')
ylabel('log of error covariance ')
...
% plot(final_errex(:,4:6))
grid on
plot(lpoles,log(final_errex(:,3)),'o-',lpoles,log(final_errex(:,4)),'x-',lpoles,log(final_errex_servo(:,3)),'o-.',lpoles,log(final_errex_servo(:,4)),'x-.')
grid on
xlabel('pole frequency [rad]')
ylabel('log of error covariance ')
legend('pos','vel','pos_{servo}','vel_{servo}')
...
% kalmanfilter
simloop = 400;
Ks = acker(sysd.A',sysd.A'*sysd.C',L)';
...
% plot(final_errex(:,4:6))
grid on
plot(lpoles,log(Final_err_real(:,3)),'o-',lpoles,log(Final_err_real(:,4)),'x-',lpoles,onespole*log(KF_err_real(3)),'--',lpoles,onespole*log(KF_err_real(4)),'-.')
grid on
xlabel('pole frequency [rad]')
ylabel('log of error covariance ')
legend('pos','vel','KF pos','KF vel')
...
%%
SaveFigPDF(4,strcat('statenoise_vsKF_acker_F',num2str(Fnum)))
SaveFigPDF(5,strcat('obsnoise_vsKF_acker_F',num2str(Fnum)))
%% Compared with KF?
...
cmap = colormap;
for lpole=1:20
plot(acbase*-1*lpoles,'*','color',cmap(lpoles*2,:))
end
legend('Controller poles','Observer poles')
xlabel('Real')
ylabel('Imaginary')
